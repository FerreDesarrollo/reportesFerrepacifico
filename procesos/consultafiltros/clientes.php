<?php
    ...
    $nombresearch=explode(" ", $_POST['nombre']);
	$where="";
    for ($i=0; $i < count($nombresearch); $i++) { 
		if($i==0)
		{
			$where.="%";
		}
		$where.="$nombresearch[$i]%";
	}
	$c=array();
	$obj = clientes([ "consulta" => "Cliente","clientepos"=>$where]);
	$obj=json_decode($obj);
	for ($i=0; $i < count($obj); $i++) { 
		...
		for ($ii=0; $ii < count($clientes1); $ii++) { 
			...
			if(isset($clientes1[$ii]->{'entityid'})){
				$id=$clientes1[$ii]->{'id'};
				$nombre=$clientes1[$ii]->{'entityid'};
